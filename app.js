...
let reducedMotion = storage.get('reducedMotion', reducedQuery.matches),
  selectedShip = 0,
  focusMode = false,
  pointer = null;
...
function chooseShip(id) {
  ...
  game.player.weapon = id === 1 ? 'laser' : 'pulse';
...
$('overdrive-button').addEventListener('click', () => {
  if (!game.overdrive() && game.mode === 'playing') toast('擦彈、擊殺和拾取武器可獲得充能');
  updateHud(true);
});
...
function syncMode() {
  ...
  $('hangar').hidden = game.mode !== 'hangar';
  $('hud').hidden = game.mode === 'hangar';
  ...
  if (game.mode === 'paused') {
    ...
    desc.textContent = pauseReason;
    actions.append(
      button(`繼續戰鬥 ${arrow}`, 'primary-button', resume),
      button('結束本次戰役，返回機庫', 'secondary-button', toHangar)
    );
  } else if (game.mode === 'upgrade') {
    kicker.textContent = `SECTOR 0${game.stageIndex + 1} / CLEAR`;
    ...
    title.textContent = won ? (game.practice ? '演練完成。' : '天穹，重歸黎明。') : '這不是最後一次出擊。';
    desc.textContent = won
      ? game.practice
        ? '單關練習已結束。準備好後，嘗試完整五關戰役。'
        : `五大空域已全部解放。${game.continues ? `本次續戰 ${game.continues} 次。` : '一命航程，一路到底。'}`
      : `抵達第 ${game.stageIndex + 1} 空域 · ${STAGES[game.stageIndex].name}。${game.continues < 3 ? '可續戰重開當前關，保留強化，分數扣除 35%。' : '本局續戰次數已用完，返回機庫再挑戰。'}`;
    $('overlay-content').innerHTML =
      `<div class="result-grid"><div><small>作戰得分</small><strong>${formatScore(game.score)}</strong></div><div><small>有效戰鬥時間</small><strong>${formatTime(game.time)}</strong></div><div><small>擊落敵機</small><strong>${game.kills}</strong></div><div><small>擦彈次數</small><strong>${game.grazes}</strong></div></div>`;
    if (!game.practice && game.score > best) {
      best = game.score;
      storage.set('best', best);
      $('best-score').textContent = formatScore(best);
    }
    if (!won && game.continues < 3)
      actions.append(
        button(`繼續作戰 <small>${3 - game.continues} 次機會</small>${arrow}`, 'primary-button', () => {
          game.continueRun();
          lastStage = -1;
          syncMode();
          updateHud(true);
    ...
    actions.append(
      button(`返回機庫 ${arrow}`, won || game.continues >= 3 ? 'primary-button' : 'secondary-button', toHangar)
    );
  }
...
function updateHud(force = false) {
  ...
  $('elapsed').textContent = formatTime(game.time);
  $('hud-stage').textContent = `${game.practice ? '演練' : '0' + (game.stageIndex + 1) + ' / 05'}`;
  ...
  if (boss) {
    $('boss-name').textContent = STAGES[game.stageIndex].bossName;
  ...
  if (lastStage !== game.stageIndex || force) {
    ...
    for (const [i, row] of [...$('mission-route').children].entries()) {
      row.classList.toggle('active', i === game.stageIndex);
      row.classList.toggle('complete', i < game.stageIndex && !game.practice);
    }
  }
  $('flight-state').textContent =
    game.mode === 'hangar' ? 'HANGAR 01' : `${game.practice ? 'TRAINING' : 'SECTOR'} 0${game.stageIndex + 1}`;
  const salvageSeconds = Math.max(
    0,
    Math.ceil(Math.max(STAGE_SECONDS, (game.bossDefeatedAt ?? 0) + DROP_TTL) - game.stageTime)
  );
...
function processEvents() {
  for (const event of game.drainEvents()) {
    ...
    if (event.type === 'boss') announce('WARNING / HOSTILE FLAGSHIP', event.name, '多階段火力 · 注意雷射預警', 2.2);
    if (event.type === 'bossDefeated') announce('FLAGSHIP DESTROYED', '空域已壓制', '繼續清場，搶收首領補給', 2);
    if (event.type === 'bossPhase') toast(`首領第 ${event.phase} 階段 · 短暫護盾後切換火力`);
    if (event.type === 'autobomb') toast('致命一擊保護 · 自動消耗 1 枚炸彈');
    if (event.type === 'overdrive') toast('雷霆爆發 / 8 秒強化火力 · 仍需躲彈');
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  input.pointer = true;
  input.targetX = game.player.x;
  input.targetY = game.player.y;
...
window.addEventListener('keydown', (e) => {
  if ($('help-dialog').open || e.target.closest('select,input,textarea') || !['playing', 'paused'].includes(game.mode))
    return;
  ...
  if (key === 'p' || key === 'escape') {
    e.preventDefault();
    game.mode === 'playing' ? pause() : resume();
  }
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    pause('你剛離開了戰場。所有計時已暫停，準備好再繼續。');
    resetInput();
  }
});
window.addEventListener('blur', () => {
  pause('視窗已失去焦點，戰鬥自動暫停。');
  resetInput();
});
$('sound-button').addEventListener('click', () => {
  sound.setEnabled(!sound.enabled);
  storage.set('sound', sound.enabled);
  sound.unlock();
  updateSound();
});
...
function frame(now) {
  ...
  if (game.mode === 'playing' && !$('help-dialog').open) {
    accumulator += dt;
    while (accumulator >= 1 / 60 && game.mode === 'playing') {
