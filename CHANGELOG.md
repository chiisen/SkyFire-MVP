...
- 武器等級上限由 Lv.5 提升至 Lv.10：1–5 級手感不變，6–10 級沿用傷害／特性成長並讓脈衝彈數上限 9 發；上限集中於 `constants.json`，HUD 等級格改為 10 格並隨面板寬度自適應，附上限與彈數回歸測試

- 穿透／追蹤／爆裂不再卡在低彈數上限：三款改為每 2 級 +1 發（滿級穿透 5 道、追蹤 6 發、爆裂 5 發），與脈衝同節奏；附彈數成長回歸測試

- 敵人改為每隻隨機配色：生成時帶一組暖色禁區色相（紅橙黃，避開冷綠背景），艦身／金屬／邊光／點綴同步旋轉並提高飽和度使差異明顯；頭目維持每關固定色；附隨機配色回歸測試

- 新增本機紀錄系統（#1）：以「模式 × 難度」保存最高分、最快通關、最高擦彈與機體；機庫可依難度切換檢視、結算標示 `NEW BEST`；舊版單一最高分自動遷移；附紀錄純邏輯回歸測試

- 暗金、暗銀、暗銅三台外觀變體改為金屬反光暗色塗裝並加專屬識別件：暗金加翼端小翼與垂直鰭、暗銀加機頭鴨翼與加長加速軌、暗銅加裝甲側裙與第三對砲管，各配亮色點綴；沿用對應正常色戰機的戰鬥數值，附識別件與亮色點綴回歸測試
...
- 三份專案規範新增 §6 Git 工作流程（PR 制）並指向新檔 `PR-WORKFLOW.md`（分支、開 PR、自我 Code Review、修正與回覆、人類合併的完整流程，可整包複製到其他專案）；`GEMINI.md` 任務查詢順延為 §7
